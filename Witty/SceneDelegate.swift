...
//        UIApplication.shared.statusBarStyle = .darkContent
        ...
        realmObjc.objects(usedVote.self).forEach({ (usedVote) in
            if usedVote.date != dateFormatterGet.string(from: Date()){
                try! realmObjc.write{
                    realmObjc.delete(usedVote)
                }
            }
        })
...
//        window?.rootViewController = UINavigationController(rootViewController: DeveloperView())
